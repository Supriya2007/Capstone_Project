...
def p_postfix_expression(p):
    '''
    postfix_expression : primary_expression #p[0] = p[1]
    | postfix_expression L_SQUARE expression R_SQUARE #name, array_size - ?
    | postfix_expression DOT IDENTIFIER     #lhs_name, rhs_name, line
    | postfix_expression PTR_OP IDENTIFIER #lhs_name, rhs_name, line
    | postfix_expression INC_OP #name, line
    | postfix_expression DEC_OP #name, line
    | function_call #p[0] = p[1]
    '''
    #ADD
    pass
...
def p_unary_expression(p):
    '''
    unary_expression : postfix_expression #p[0] = p[1]
    | INC_OP unary_expression #p[0].exp = INC_OP+p[1].exp?
    | DEC_OP unary_expression   #p[0].exp = DEC_OP+p[1].exp?
    | unary_operator cast_expression    #p[0].exp = p[1].exp + p[2].exp?
    | SIZEOF unary_expression   #p[0].exp = p[1]+p[2].exp?
...
def p_assignment_expression(p):
    ...
    '''
    print("assignment_expression:", p[0], p[1])
    #ADD
...
def p_variable_declaration1(p):
    ...
    LINE = p.lineno(1)
    #p[0] = (NAME, LINE)
    print("variable_declaration1:", p[0], p[1])
    #2nd & 3rd rules - arrays, 4th rule - function pointers
    #ADD   
...
def p_compound_statement(p):
    ...
    | L_BRACE statement_list R_BRACE    #p[0]['statements'] = p[2]
    | L_BRACE declaration_list R_BRACE
    | L_BRACE declaration_list statement_list R_BRACE   #p[0]['statements'] = p[3]
    '''
    pass
...
def p_statement_list(p):
    ...
    '''
    p[0] = {}
    p[0]['statements'].append(statement) #append to a list
    pass
