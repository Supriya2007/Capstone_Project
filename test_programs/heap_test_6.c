...
//test file: comment lines to check each possible error msg

void insertNode(int my_heap[], int n, int key){
    int child_ind = n+1;
    //int parent_ind = child_ind/2;
    int parent_ind;
    parent_ind =  child_ind/2; //line-1
    //int parent_ind = (n+1)/2;
    while(child_ind > 1 && my_heap[parent_ind] < key){
        my_heap[child_ind] = my_heap[parent_ind]; //line-2
        child_ind = parent_ind; //line-3
        parent_ind = parent_ind/2;  //line-4
    }
