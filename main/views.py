...
#file_count = 0
#code_file_count = 0
#fs_files = [] #fs = formal structure
#src_code_fname = 'Code.c'
#selected_lib_rules = []
#new_fs_rules={}
#code_files = []
#chosen_guideline_names=[]
#chosen_guideline_names={}
#guideline_vars = {}
#chosen_rules_with_vars = {}
#input_var_values = {}
#selected_lib_rules_with_vars = {}
#rule_names = {}
#lines=dict()
#v=dict()
#g=dict()
...
def handle_uploaded_file(f, newfname):
    #with open('Code.c', 'wb+') as destination:
    with open(newfname, 'wb+') as destination:
    #Get name of file same as users file
...
def process_guideline_variables(rule_names, guideline_vars):
    #global guideline_vars
    #guideline_vars = {}
    processed_guidelines = {}
    #new_rule_names = copy.deepcopy(rule_names)
    ...
    for i in range(len(rule_names)):
        ...
        #new_rule_names[cur_rule]=cur_rule unless input tags need to be inserted
...
def home(request):
    ...
    #rule_names = list(lib_rules.keys())
    #rule_names = {} #rule : rule_with_input_tags
    #insert_input_tags(rule_names)
    ...
    #return render(request, 'index_new.html', {"cmd_info":text, "guideline_names":rule_names, "chosen_guideline_names":[]})
...
def home_reload(request):
    ...
    text = fh.read()
    fh.close()
    #return render(request, 'index.html', {"cmd_info":text})
    ...
    #return render(request, 'index_new.html', {"cmd_info":text, "guideline_names":['Rule1', 'Rule2', 'Rule3']})
    #Get guidline names from as dict.values, where dict = {'guideline_name':'formal strcut file name'}
...
def get_prog_files(request):
    ...
    #handle_uploaded_file(code_file, src_code_fname)
    #response = redirect('home') #name of function given in urls.py
...
def get_selected_lib_rules(request):
    #global selected_lib_rules_with_vars
    ...
    #chosen_guideline_names.update(selected_lib_rules_with_vars)
...
def get_formal_struct(request):
    ...
    file_count+=1
    #fname = '%s%sfs_%d.txt'%(fs_dir, file_sep, file_count)
    ...
    if(fs):
        ...
        #Take from text area if Formal Structure file not uploaded
        fs_text = request.POST['fs_textarea']
        #print("fs_text", fs_text)
    ...
    #chosen_guideline_names.update({gname:[]})
    #chosen_rules_with_vars.update({gname:[]})
    #response = redirect('home') #name of function given in urls.py
    response = redirect('home_reload')
...
def get_data(request):
    ...
    print(os.getcwd())
    print(request.FILES)
    #print(request.FILES['code_file'])
    code_f = request.FILES['code_file']
    handle_uploaded_file(code_f, 'Code.c')
    #print(request.POST['lib_rule'])
    #handle_uploaded_file(request.FILES['code_file'])
    #print(request.FILES.get('fs_file', [1]))
    if(request.FILES.get('fs_file', [])):
        formal_struct_f = request.FILES['fs_file']
        handle_uploaded_file(formal_struct_f, 'fs.txt')
        formal_struct_f = request.FILES['fs_file']
        guideline_name = request.POST.get('new_rule_name')
        new_fs_rules = {guideline_name : 'fs.txt'}
    else:
        new_fs_rules = {}
    #handle_uploaded_file(formal_struct_f, 'fs.txt')
    #print(request.POST.getlist('lib_rule'))
    lib_rules = request.POST.getlist('lib_rule') 
    '''
    #code_files = ['Code.c']
    #code_files = ['Code.c', 'gf.c', 'func_len.c', 'var_len.c', 'no_a.txt']
    #guideline_name = request.POST.get('new_rule_name')
    #new_fs_rules = {guideline_name : 'fs.txt'}
    new_fs_rules = request.session['new_fs_rules']
    code_files = request.session['code_files']
    ...
    #violations = Run_Formal_Structs.main(lib_rules, fs_files, code_files)
    selected_lib_rules = request.session['selected_lib_rules']
    chosen_guideline_names = request.session['chosen_guideline_names']
    print("selected_lib_rules", selected_lib_rules)
    #violations = Run_Formal_Structs.main(selected_lib_rules, new_fs_rules, code_files)
    print("selected_lib_rules_with_vars:", selected_lib_rules_with_vars)
    ...
    #print(contents)
    #f.close()
    #return render(request, 'page2.html',{"content":contents,"line":line,"v":v})
    print(v)
    #clear_global_vars()
    print("g=",g)
    print("v=",v)
